...
class MY_Session extends CI_Session {
	...
	function sess_read()
	{
		...
		{
			$this->CI->mongo_db->where(array('session_id'=>$session['session_id']));

			if ($this->sess_match_ip == TRUE)
			{
				$this->CI->mongo_db->where(array('ip_address'=>$session['ip_address']));
			}

			if ($this->sess_match_useragent == TRUE)
			{
				$this->CI->mongo_db->where(array('user_agent'=>$session['user_agent']));
			}

			$query = $this->CI->mongo_db->get($this->sess_collection_name);
	...
	function sess_write()
	{
		...
		$this->CI->mongo_db->update($this->sess_collection_name, array('last_activity' => $this->userdata['last_activity'], 'user_data' => $custom_userdata));
	...
	function sess_create()
	{
		...
		if ($this->sess_use_mongo === TRUE)
		{
			$this->CI->mongo_db->insert($this->sess_collection_name, $this->userdata);
		}
	...
	function sess_update()
	{
		...
		if ($this->sess_use_mongo === TRUE)
		{
			...
			$this->CI->mongo_db->where(array("session_id"=>$old_sessid))->update($this->sess_collection_name, array('last_activity' => $this->now, 'session_id' => $new_sessid));
		}
	...
	function sess_destroy()
	{
		...
		{
			$this->CI->mongo_db->where(array('session_id'=>$this->userdata['session_id']));
			$this->CI->mongo_db->delete($this->sess_collection_name);
		}
	...
	function _sess_gc()
	{
		...
		if ((rand() % 100) < $this->gc_probability)
		{
			...
			$this->CI->mongo_db->where(array("last_activity" =>array('$lt'=>$expire)));
			$this->CI->mongo_db->delete($this->sess_collection_name);
