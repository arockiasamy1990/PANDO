...
#require_once('MongoDate.php');
#require_once('MongoDB\BSON\ObjectId.php');
/**
...
Class Mongo_db{
	...
	*/
	...
	private function connect()
	{
		...
		try
		{
			...
			if(!empty($this->config[$this->activate]['server_type']) && $this->config[$this->activate]['server_type']=='atlas'){
				$dns = "mongodb+srv://{$this->hostname}/{$this->database}";
	...
	*/
	public function insert($collection = "", $insert = array())
	{
		...
		try
		{
            ...
            $this->_inserted_id=(string)$insert['_id'];
	...
	*/
	public function like($field = "", $value = "", $flags = "i", $enable_start_wildcard = TRUE, $enable_end_wildcard = TRUE)
	{
		...
		$this->wheres[$field] = new MongoDB\BSON\Regex("$like","$flags");
		return ($this);
	}
    ...
    public function or_like($field, $like = array(),$flags = "i") {
        ...
        if (is_array($like) && count($like) > 0) {
            foreach ($like as $admitted) {
                $this->wheres['$or'][] = array($field => new MongoDB\BSON\Regex("$like","$flags"));
            }
        } else {
            $this->wheres['$or'][] = array($field => new MongoDB\BSON\Regex("$like","$flags"));
        }
	...
	*/
	...
	{			
		...
		try{	
            ...
            $cimongo_cursor = new Cimongo_cursor($cursor_array);
	...
	*/
	public function aggregate($collection, $operation)
	{
		...
		$command = array('aggregate'=>$collection, 'pipeline'=>$operation,'cursor' => array('batchSize' => 100000));
	...
	*/
	 ...
	 *  @return  stdClass
	 */
	public function convert_document_id($document)
	{
	...
	*/
    ...
    public function command($command = array())
    {
		try{
			...
			$cursor = $this->db->executeCommand($this->database, new MongoDB\Driver\Command($command),array('options'=>array('unique'=>TRUE)));
			// Clear
			$this->_clear();
			$returns = array();
			// Clear
    ...
    public function writecommand($command = array())
    {
		try{
            $options = [];
            $writeConcern = new MongoDB\Driver\WriteConcern(MongoDB\Driver\WriteConcern::MAJORITY, 1000);
            $options['writeConcern'] =$writeConcern;
			$cursor = $this->db->executeWriteCommand($this->database, new MongoDB\Driver\Command($command),$options);
			$this->_clear();
            $cursor->setTypeMap(array(
                  'array' => 'array',
                  'document' => 'array',
    ...
    public function findAndModify($collection, $query=array(),$sort=array(),$updateArr=array(),$includes=array())
	{
        if (empty($collection))
	 	{
	 		show_error("In order to retreive documents from MongoDB, a collection name must be passed", 500);
		...
		$command = array('findAndModify'=>$collection, 'query'=>$query,'update'=>array('$set'=>$updateArr),'new'=>true);
	...
	*/
	public function ensure_index($collection = "", $keys = array(), $options = array())
    {
        ...
        foreach ($keys as $col => $val) {
            ...
            if($val == -1 || $val === FALSE || strtolower($val) == 'desc'){
            $keys[$col] = -1;
                $index_array = array('name'=>$col."_desc",'key'=>array("$col"=>-1),'ns'=>"$this->database.$collection");
            } else if($val == 1 || $val === FALSE || strtolower($val) == 'asc') {
                $index_array = array('name'=>$col."_asc",'key'=>array("$col"=>1),'ns'=>"$this->database.$collection");
            } else {
                $index_array = array('name'=>$col."_$val",'key'=>array("$col"=>$val),'ns'=>"$this->database.$collection");
            }
        }
        $commond = array();
        $commond['createIndexes'] = $collection;
        $commond['indexes'] = array(array_merge($index_array,$options));
        return $this->command($commond);
    }
